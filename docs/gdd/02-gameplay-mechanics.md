---
title: Gameplay & Mechanics
layout: page
---

# Section II – Gameplay & Mechanics

## 1. Gameplay

### 1.1 Game Progression
*Describe progression mechanics.*

### 1.2 Mission/Challenge Structure
*Details on missions or challenges.*

### 1.3 Puzzle Structure
*If applicable.*

### 1.4 Objectives
*What are the objectives?*

### 1.5 Play Flow
*How the game flows for the player.*

## 2. Mechanics

### 2.1 Physics
*Describe the physical rules of the game world.*

### 2.2 Movement
#### 2.2.1 General Movement
#### 2.2.2 Other Movement

### 2.3 Objects
#### 2.3.1 Picking Up Objects
#### 2.3.2 Moving Objects

### 2.4 Actions
#### 2.4.1 Switches and Buttons
#### 2.4.2 Picking Up, Carrying and Dropping
#### 2.4.3 Talking
#### 2.4.4 Reading

### 2.5 Combat
*How combat/conflict is modeled.*

### 2.6 Economy
*How the game economy works.*

## 3. Screen Flow

### 3.1 Screen Flow Chart
*Graphical description of screens.*

### 3.2 Screen Descriptions
#### 3.2.1 Main Menu Screen
#### 3.2.2 Options Screen
#### 3.2.3 Etc.

## 4. Game Options
*How options affect gameplay.*

## 5. Replaying and Saving
*Details on saving/replaying.*

## 6. Cheats and Easter Eggs
*Optional content.*
